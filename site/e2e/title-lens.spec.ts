...
test('title lens: expand seniority, click a registry SOC segment, role + map update', async ({ page }) => {
  ...
  // Role select's value changed, and the URL gained the role param. Exact match keeps this
  // pinned to the filter bar's own "Role" label rather than any other accessible name that
  // happens to contain the substring "Role".
