...
import { gazetteerRowsToMap } from './lib/parse-gazetteer'
import { hudRowsToZipCbsa } from './lib/crosswalk'
import { lcaRowsToRecords, type LcaRecord } from './lib/parse-lca'
...
console.log(`  RPP ${rpp.size} metros · gazetteer ${coords.size} CBSAs · HUD ${zipCbsa.size} ZIPs`)
...
for (const f of lcaFiles) {
  const recs = lcaRowsToRecords(readSheetRows(path.join(RAW_DIR, f)))
  console.log(`  ${f}: ${recs.length} usable filings`)
  lcaRecords.push(...recs)
}
const { matched, matchRate, unmatchedZips } = attachCbsa(lcaRecords, zipCbsa)
...
const { meta, dropped } = buildMeta(salaries, areas, coords, rpp, year)
...
writeFileSync(path.join(OUT_DIR, 'salaries.json'), JSON.stringify(buildSalaries(salaries)))
const employerFiles = buildEmployerFiles(aggregateEmployers(matched))
...
writeFileSync(path.join(REPORT_DIR, `run-${meta.generated.slice(0, 10)}.json`), JSON.stringify({
  generated: meta.generated, year, oewsFile: path.basename(oewsFile), lcaFiles,
  metros: meta.metros.length, metrosDroppedNoCoords: dropped, salaryRows: salaries.length,
  lcaUsable: lcaRecords.length, lcaMatched: matched.length, zipMatchRate: matchRate,
